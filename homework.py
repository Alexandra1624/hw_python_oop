...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        spent_calories: float = (
            (self.COEFF_WLK_1 * self.weight + (self.get_mean_speed()
             ** self.DEGREE // self.height) * self.COEFF_WLK_2 * self.weight)
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        spent_calories: float = (
            (self.get_mean_speed() + self.COEFF_SWM_1) * self.COEFF_SWM_2
            * self.weight
        )
        return spent_calories
